...
const MenagePost = () => {
  const { data, error, isLoading, refetch } = useGetPostQuery(undefined, {
    ...
    <div className="overflow-x-auto w-[95%] mx-auto">
      <div className="flex justify-end mb-6 pt-4">
        <Link
          ...
          className="bg-green-500 py-3 px-4 text-white font-semibold rounded-md shadow-md lg:text-lg hover:bg-green-600 "
        >
          {" "}
          Create New News
        </Link>
      </div>
      <p className="text-center font-semibold text-lg md:text-2xl lg:text-3xl mb-6">
      ...
      {data?.data?.length ? (
        data?.data?.map((post) => (
          <table
            key={post._id}
            className="min-w-full bg-white border border-gray-200 text-black mb-6"
          >
            <thead>
              <tr>
                <th className="text-left py-3 px-4 bg-gray-100 border-b border-gray-200">
                  Field
                </th>
                <th className="text-left py-3 px-4 bg-gray-100 border-b border-gray-200">
                  Value
            ...
            <tbody>
              <tr>
                <td className="py-3 px-4 border-b border-gray-200 font-semibold">
                  Post ID
                </td>
                ...
                <td className="py-3 px-4  border-gray-200 ">
                  ...
                  <button
                    ...
                    className="mr-2 bg-yellow-500 hover:bg-yellow-600 text-white px-2 py-1 rounded-md my-2 md:mt-0 "
                  >
